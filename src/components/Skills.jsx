...
const Skills = () => {
  ...
  return (
    <section id="skills" className="py-12 sm:py-20 bg-secondary/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
          {skillCategories.map((category, index) => (
            <Card key={index} className="p-4 sm:p-6 glass-card border-border/50">
        ...
        <Card className="p-6 sm:p-8 glass-card border-border/50">
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {technologies.map((tech, index) => (
              <div
                key={index}
                className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20 hover:border-primary/40 hover:-translate-y-1 transition-all duration-300 cursor-default group"
              >
                <span className="font-medium group-hover:text-primary transition-colors text-xs sm:text-sm">
                  {tech}
                </span>
              </div>
            ))}
